...
export function mockContainerMetrics(
  ...
  return {
    ...
    networkInSeries: walk(rand, networkIn, points, networkIn * 0.4, 0, Infinity),
    ...
    diskWriteSeries: walk(rand, diskWrite, points, diskWrite * 0.5, 0, Infinity),
...
export function mockEnvVars(containerId: string): MockEnvVar[] {
  ...
  return [
    ...
    { key: 'DATABASE_URL', value: 'postgres://••••••••@db:5432/app', masked: true },
    { key: 'TZ', value: 'UTC' },
  ]
}
...
export function mockNetworks(hostId: string): MockNetwork[] {
  ...
  return names.map((name, index) => ({
    ...
    scope: 'local',
    subnet: name === 'host' || name === 'none' ? '—' : `172.${17 + index}.0.0/16`,
